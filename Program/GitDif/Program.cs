...
namespace GitDif
{
    class Program
    {
        private static TextFiles User_Input_Class_Instantiation;
        static void Main()
        {
            ...
            while (!Leave_Loop) //Error checking User's command and parameter inputs - only allowing 'dif' command and valid text file names beginning with "GitRepositories_" and being of type ".txt"
            {
                try
                {
                    ...
                    {
                        User_Input_Class_Instantiation = new TextFiles(User_Input);
                        First_Contents = User_Input_Class_Instantiation.FirstIndividualCharacters();
                        Second_Contents = User_Input_Class_Instantiation.SecondIndividualCharacters();
                        Leave_Loop = true;
                    }
                    else
                    {
